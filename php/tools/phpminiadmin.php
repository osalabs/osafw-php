...
function frefresh(set_db){
 var F=document.DF;
 F.method='get';
 F.refresh.value="1";
 if (set_db) F.db.value="";
 F.GoSQL.click();
}
function go(p,sql){
 var F=document.DF;
...
<?php if ($_SESSION['is_logged'] && $dbh){
 if ($DBSERVERS){?>
 | Servers: <select name="srv" onChange="frefresh(true)"><option value=''>- select/refresh -</option>
<?php echo @sel($DBSERVERS,'iname',$SRV)?></select>
<?php }
?>
&#183; <a href="?<?php eo("$xurl&db=".ue($dbn)."&srv=".ue($SRV).'&q='.b64u("show processlist"))?>">ps</a>
 | <a href="?<?php eo($xurl."&srv=".ue($SRV).'&q='.b64u("show databases"))?>">Databases</a>: <select name="db" onChange="frefresh()"><option value='*'> - select/refresh -</option><option value=''> - show all -</option>
...
function get_db_select($sel=''){
 global $DB,$SHOW_D;
 $ck='sql_sd#'.$DB['host'];
 if (is_array($_SESSION[$ck]??0) && ($_REQUEST['db']??'')!='*'){//check cache
    $arr=$_SESSION[$ck];
 }else{
...
function loadsess(){
 ...
 if($SRV) {
   foreach ($DBSERVERS as $v) {
     if($v['iname']==$SRV) {
       #if($DB['user'] . '|' . $DB['host'] != $v['config']['user'] . '|' . $v['config']['host']) $rdb = '';#reset db if host changed (done in js)
       $DB=$v['config'];
       break;
     }
   }
