...
function modal_alert(content, title){
...
/*
modal_confirm('Are you sure?', 'optional title', function(){
  //proceed OK answer
});
 */
function modal_confirm(content, title_or_cb, callback){
...
function setup_fw_handlers() {
  //list screen init
  make_table_list(".list");

  //autosubmit filter on change
  $('#FFilter[data-autosubmit]').on('change', 'select, input[type="radio"], input[type="checkbox"]', function(){
      $('#FFilter').trigger('submit');
  });

  //pagesize change
  $(document).on('change', '.on-pagesize-change', function (e){
      $('#FFilter input[name="f[pagesize]"]').val($(this).val());
      $('#FFilter').submit();
  });
  ...
  $(document).on('click', '.on-delete-multi', function (e){
    ...
    if (!el._is_confirmed){
      e.preventDefault();
      modal_confirm('Are you sure to delete multiple selected records?', function(){
...
function cancel_form(f, url){
  ...
  if ( $f.data('is-changed')===true ){
    modal_confirm('<strong>There are unsaved changes.</strong><br>Do you really want to Cancel editing the Form?', function (){
...
// <form data-autosave>
function setup_autosave_form_handlers() {
...
function delete_btn(ael){
  modal_confirm('<strong>ARE YOU SURE</strong> to delete this item?', function(){
...
// depends on #FFilter
// <table class="list" data-rowtitle="Double click to Edit" [data-rowtitle-type="explicit"]>
//  <thead>
