...
#require_once dirname(__FILE__) . '/../vendor/autoload.php'; #uncomment if Composer libs used
...
class UserException extends ApplicationException {} #more specific exception - this one should be passed to user
...
class fw {
    ...
    public static $LOG_LEVELS = array(
        ...
        'NOTICE' => 50, #normal, but significant noticeable condition
        'DEBUG'  => 60, #default for development (default for logger("msg") call), fine-grained level
        'TRACE'  => 70, #very detailed  (in-module details like fw core, despatcher, parse page, ...)
        'ALL'    => 80, #just log everything
    );
    ...
    public static function run($ROUTES = array()) {
        ...
        logger('INFO', "*** REQUEST START [$method $uri]");
        ...
        logger('INFO','*** REQUEST END in ' . number_format($total_time, 10) . 's, ' . number_format(1 / $total_time, 3) . '/s, SQL: ' . DB::$SQL_QUERY_CTR);
    }
    ...
    public function runRoute() {
        try {
            ...
            $this->handleRouteError(401, $ex->getMessage(), $ex);
            ...
            try {
                ...
                $this->handleRouteError(404, $ex->getMessage(), $ex);
                return;
            }
        } catch (NoClassException $ex) {
            #if can't call class - this is server error
            $this->handleRouteError(500, $ex->getMessage(), $ex);
            return;
            ...
            $this->handleRouteError(($ex->getCode() ? $ex->getCode() : 500), $ex->getMessage(), $ex);
        } catch (Exception $ex) {
            $this->handleRouteError(500, $ex->getMessage(), $ex);
        }
    }
    ...
    public function getResponseExpectedFormat($route = null) {
        ...
        //might need to also check $_SERVER['HTTP_X_REQUESTED_WITH']
        if ($route->format == 'json' || preg_match('!application/json!', $_SERVER['HTTP_ACCEPT'])) {
            $result = 'json';
    ...
    #use in controllers POST methods
    ...
    public function handleRouteError($error_code, $error_message = '', $exeption = null) {

        if ($exeption) {
            $ps = FwHooks::handleRouteException($exeption);
            if ($ps) {
        ...
        if ($exeption) {
            ...
            logger('ERROR', "Dispatcher - handleRouteError : $error_code $error_message");
        }

        if ($error_code >= 500 && !is_null($exeption)) {
            logger($exeption->getTraceAsString());
        }
    ...
    public function parser() {
        ...
        if ($out_format == 'json') {
            if (isset($ps['_json'])) {
                ...
                logger("WARN", "JSON response is not enabled for the Controller.Action (set ps[_json]=true to enable).", array($_SERVER, $_REQUEST));
...
function _req($name) {
    if (preg_match('/^(.+?)(?:\[(.+?)\])/', $name, $m)) {
        return isset($_REQUEST[$m[1]]) ? (isset($_REQUEST[$m[1]][$m[2]]) ? $_REQUEST[$m[1]][$m[2]] : '') : '';
    } else {
...
########################## for debug
# IN: [logtype (ALL|TRACE|DEBUG|INFO|WARN|ERROR|FATAL), default DEBUG] and variable number of params
