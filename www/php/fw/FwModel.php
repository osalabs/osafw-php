...
abstract class FwModel {
    ...
    public function one($id) {
        ...
        if (is_null($row)) {
            $row = $this->db->row($this->getTable(), array($this->field_id => $id));
    ...
    public function oneByIname($iname) {
        return $this->db->row($this->getTable(), array($this->field_iname => $iname));
    }

    public function listFields() {
        $rows = $this->db->arr("EXPLAIN " . $this->db->quote_ident($this->getTable()));
    ...
    public function delete($id, $is_perm = null) {
        if ($is_perm || !strlen($this->field_status)) {
            ...
            $vars = array(
                $this->field_status => self::STATUS_DELETED,
            );
            $this->update($id, $vars);
        }
    ...
    public function ilist() {
        ...
        return $this->db->arr($this->getTable(), $where, $this->db->quote_ident($this->field_iname));
    }

    public function listSelectOptions() {
        $where = '';
        if (strlen($this->field_status)) {
            $where .= " " . $this->db->quote_ident($this->field_status) . "=" . dbqi(self::STATUS_ACTIVE);
        }
    ...
    public function getCount() {
        $where = '';
        if (strlen($this->field_status)) {
            $where .= " " . $this->db->quote_ident($this->field_status) . "<>" . dbqi(self::STATUS_DELETED);
        }

        return $this->db->value("SELECT count(*) FROM " . $this->db->quote_ident($this->getTable()) . " WHERE $where");
    }
    ...
    public function getAutocompleteList($q, $limit = 5) {
        ...
        if (strlen($this->field_status)) {
            $where .= " and " . $this->db->quote_ident($this->field_status) . "<>" . dbqi(self::STATUS_ACTIVE);
        }
