...
class AdminSpagesController extends FwAdminController {
    ...
    public function ShowFormAction($form_id) {
        ...
        if ($id) $ps["subpages"] = $this->model->listChildren($id);
    ...
    public function SaveAction($form_id) {
        ...
        try{
            ...
            if (!$itemdb["head_att_id"]) $itemdb["head_att_id"] = null;
            logger($itemdb);

            $id = $this->modelAddOrUpdate($id, $itemdb);

            if ($item_old["is_home"]==1) FwCache::remove("home_page"); #reset home page cache if Home page changed
